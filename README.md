...
Модель `AppState` представляет собой глобальное состояние приложения, которое содержит информацию о корзине, магазине, заказе, ошибках формы и других данных.

Свойства:
* `basket`: Массив товаров в корзине.
* `store`: Массив товаров в магазине.
...
* `setItems()`: Устанавливает список идентификаторов товаров в заказе.
* `setOrderField(field: keyof IOrderForm, value: string)`: Устанавливает значение поля в заказе.
* `validateContacts()`: Проверяет корректность введенных данных о контактах.
* `validateOrder()`: Проверяет корректность введенных данных о заказе.
* `refreshOrder()`: Сбрасывает информацию о заказе.
* `getTotalBasketPrice()`: Возвращает общую стоимость товаров в корзине.
* `setStore(items: IProduct[])`: Устанавливает список товаров в магазине.
* `resetSelected()`: Сбрасывает флаг `selected` для всех товаров в магазине.

События:
* `contacts:ready`: Срабатывает, когда данные о контактах валидны.
* `order:ready`: Срабатывает, когда данные о заказе валидны.
* `contactsFormErrors:change`: Срабатывает, когда меняются ошибки формы о контактах.
* `orderFormErrors:change`: Срабатывает, когда меняются ошибки формы о заказе.
* `items:changed`: Срабатывает, когда меняется список товаров в магазине.
